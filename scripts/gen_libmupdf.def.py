...
def main():
	...
	app_exports = collectFunctions("platform/x11/pdfapp.c")
	exclude_exports = ["fz_get_annot_type","fz_getoptw","pdf_access_submit_event","pdf_crypt_buffer","pdf_drop_designated_name","pdf_init_ui_pointer_event","pdf_open_compressed_stream","pdf_read_pfx","pdf_resolve_obj","pdf_signer_designated_name","fz_colorspace_name_process_colorants",
"fz_set_stderr",
"fz_set_stdout",
"fz_stderr",
"fz_valgrind_pixmap",
...
"pdf_recognize",
"track_usage"]
	fitz_exports = generateExports("include/mupdf/fitz", doc_exports + more_formats + misc_exports + exclude_exports)
	mupdf_exports = generateExports("include/mupdf/pdf", form_exports + sign_exports + exclude_exports + ["pdf_drop_designated_name", "pdf_print_xref", "pdf_recognize", "pdf_resolve_obj", "pdf_open_compressed_stream", "pdf_finish_edit"])
	
	pdfapp_exports = generateExports("include/mupdf/pdf", exclude_exports)

	pdfapp_fz_exports = generateExports("include/mupdf/fitz", exclude_exports)

	list = LIBMUPDF_DEF % locals()
	
	str = distinct(list.split())
	
	list = MUPDFLIB_DEF % locals()
	open("platform/win32/libmupdf.def", "wb").write(list)
	#open("platform/win32/libmupdf.def", "wb").write(list.replace("\n", "\r\n"))
	#open("platform/win32/libmupdf.def", "wb").write(lst)
